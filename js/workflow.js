...
function createMethodWorkflow() {
  ...
  gradient.append("stop").attr("offset", "0%").attr("stop-color", "#3498db");
  ...
  const stages = [
    ...
    },
    {
      ...
      description:
        "YOLO + CWA-SAM for precise animal & environment segmentation",
      icon: "camera",
      x: width * 0.38,
      y: height * 0.5,
      color: "#2ecc71",
    ...
    },
    {
      ...
      description:
        "LLM-based natural language descriptions with knowledge graph",
      icon: "comment-alt",
      x: width * 0.85,
      y: height * 0.5,
      color: "#e74c3c",
  ...
  connections.forEach((connection) => {
    ...
    const path = connectionGroup
      ...
      .attr(
        "d",
        `M${source.x},${source.y} C${controlX1},${controlY1} ${controlX2},${controlY2} ${target.x},${target.y}`
      )
      .attr("fill", "none")
  ...
  const stageGroup = svg.append("g").attr("class", "stage-group");
  ...
  stages.forEach((stage, index) => {
    ...
    group
      .on("mouseover", function () {
        // Highlight the stage
        circle.transition().duration(300).attr("r", 55).attr("stroke-width", 4);

        // Show info box
        infoBox.transition().duration(300).attr("opacity", 1);
      ...
      .on("mouseout", function () {
        // Reset the stage
        circle.transition().duration(300).attr("r", 50).attr("stroke-width", 3);

        // Hide info box
        infoBox.transition().duration(300).attr("opacity", 0);
  ...
  labelGroup
    ...
    .attr("fill", "#7f8c8d")
    .text(
      "End-to-end Knowledge Graph-based automated identification of Wildlife Behavior"
    )
    .attr("opacity", 0)
    .transition()
    .delay(1200)
...
function addAnimatedMarkers(svg, path, startColor, endColor) {
  ...
  const color = d3.scaleLinear().domain([0, 1]).range([startColor, endColor]);
...
function animateMarker(marker, pathNode, pathLength, delay, colorScale) {
  ...
  setTimeout(() => {
    // Make marker visible
    marker.attr("opacity", 1).attr("cx", 0).attr("cy", 0);

    // Start animation
    (function repeat() {
      marker.attr("opacity", 0).attr("transform", "translate(0, 0)");
